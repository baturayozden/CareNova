...
export default function ProblemSection() {
  ...
  return (
    <section id="problem" className="relative py-24 bg-brand-900/[0.03]">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} variants={stagger(0.12)} className="grid md:grid-cols-3 gap-6">
          {cards.map(c => (
            <motion.div key={c.title} variants={fadeUp} className="rounded-2xl bg-surface border border-brand-900/10 p-8 shadow-sm">
              <p className="font-display text-3xl text-accent-700 mb-4">{c.stat}</p>
