...
export default function FAQSection() {
  ...
  return (
    <section id="faq" className="relative py-24 bg-brand-900/[0.03]">
      <div className="mx-auto max-w-3xl px-6">
        ...
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.1 }} variants={stagger(0.06)} className="space-y-3">
          {items.map((item, i) => {
            ...
            return (
              <motion.div key={item.q} variants={fadeUp} className="rounded-xl border border-brand-900/10 bg-surface overflow-hidden">
                <button
                ...
                >
                  ...
                  <span className={`shrink-0 text-accent-500 transition-transform duration-200 ${isOpen ? 'rotate-45' : ''}`}>+</span>
                </button>
