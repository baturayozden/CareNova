...
export default function BranchesPage() {
  ...
  return (
    <div className="space-y-4">
      <AppMeta title={`${tr('branches.title')} | CareNova Platform`} />
      <div>
        <h1 className="text-xl font-semibold text-ink">{tr('branches.title')}</h1>
        <p className="text-ink-muted text-sm mt-0.5">
          {tr('branches.subtitleBefore')}
          <code className="mx-1 text-xs bg-surface-sunken px-1.5 py-0.5 rounded">ai_pricing_authority</code>
          {tr('branches.subtitleAfter')}
        </p>
      </div>
          ...
          return (
            <div key={t.key}>
              ...
              >
                ...
                <StatusBadge tone={t.fullyAuthored ? 'success' : 'neutral'}>{t.fullyAuthored ? tr('branches.readyTemplate') : tr('branches.configurable')}</StatusBadge>
                <span className="text-xs text-ink-muted">{AUTHORITY_LABELS[t.aiPricingAuthority]}</span>
                <span className="ml-auto text-xs text-ink-subtle">{tr('branches.usageCount', { count: usage })}</span>
              </button>
              {isOpen && (
                <div className="px-4 pb-4 pl-14 space-y-3 text-sm">
                  {t.knowledgeSeedNote && (
                    <div className="rounded-lg border border-danger/30 bg-danger-soft p-3 flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-xs font-semibold text-danger mb-0.5">{tr('branches.lockedRuleTitle')}</p>
                        <p className="text-xs text-ink">{t.knowledgeSeedNote}</p>
                      </div>
                    </div>
                  )}
                  <div>
                    <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1">{tr('branches.aiPricingAuthorityLabel')}</p>
                  ...
                  {t.preAssessmentQuestions.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1">{tr('branches.preAssessmentQuestionsLabel')}</p>
                  ...
                  {t.requiredMedia.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1">{tr('branches.requiredMediaLabel')}</p>
                      <p className="text-ink-muted text-xs">{t.requiredMedia.join(' · ')}</p>
                    </div>
                  )}
                  {t.redFlags.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1">{tr('branches.redFlagsLabel')}</p>
                      <p className="text-danger text-xs">{t.redFlags.join(' · ')}</p>
                    </div>
                  )}
                  {t.branchObjections.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1">{tr('branches.branchObjectionsLabel')}</p>
                      <p className="text-ink-muted text-xs">{t.branchObjections.join(' · ')}</p>
                    </div>
                  )}
                  <div>
                    <p className="text-xs font-semibold text-ink-subtle uppercase tracking-wide mb-1">{tr('branches.aftercareScheduleLabel')}</p>
